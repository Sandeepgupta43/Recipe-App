    ...
    useEffect(() => {
        const checkIfSaved = async () => {
            try {
                ...
                console.log("Error while checking if recipe is saved:", error);
            }
        };

        const loadRecipeDetail = async () => {
            try {
                const mealData = await MealApi.searchMealById(recipeId);
                ...
                if (mealData) {
                    const transformData = MealApi.transformMealData(mealData);
                ...
                console.log("Error while loading the recipe Details", error);
            } finally {
                setLoading(false);
            }
        };
        ...
        try {
            if (isSaved) {
                const response = await fetch(
                    `${API_URL}/api/favorite/${userId}/${recipeId}`,
                    {
                        method: "DELETE",
                    }
                );
            ...
            console.log("❌ Error while saving/removing recipe:", error);
